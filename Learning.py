...
def reconstruct_tip(frequencies:tuple , theta_points):
    ...
    Will only sample points that are already in theta_points argument.
    '''
    circles = Drawing(600 , 600)
    actual_frame = np.ones((height, width, 3), dtype=np.uint8) * 255
    
    actual_frame = circles.draw_all_circles_once(theta_points)#does a single rotation
...
#rounding theta down 
unique_df['theta'].apply(lambda x : round(x , 3))
